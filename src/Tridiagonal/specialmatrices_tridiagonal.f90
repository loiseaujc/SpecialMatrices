module specialmatrices_tridiagonal
   ...
   type, public :: Tridiagonal
      !! Base type used to define a `Tridiagonal` matrix of size `[n, n]` with diagonals
      !! given by rank-1 arrays `dl(n)`, `dv(n)` and `du(n-1)`.
      private
      integer(ilp) :: n
   ...
   interface matmul
      !! This interface overloads the Fortran intrinsic `matmul` for a `Tridiagonal` matrix.
      ...
      !! `Tridiagonal` matrix. Both \( B \) and \( C \) need to be standard Fortran rank-2
      !! arrays. All the underlying functions are defined as `pure`.
      !!
      !! #### Syntax
      !!
      !! - For matrix-vector product with `A` being of type `Tridiagonal` and `x` a standard
      !! rank-1 array:
      !! ```fortran
      !!    y = matmul(A, x)
      !! ```
      !!
      !! - For matrix-matrix product with `A` being of type `Tridiagonal` and `B` rank-2 array:
      !! ```fortran
      !!    C = matmul(A, B)
      !! ```
      ...
      pure module function spmvs(A, x) result(y)
         !! Compute the matrix-matrix product \(Y = Ax\) for a `Tridiagonal` matrix \(A\) and a
   ...
   interface solve
      ...
      !! system \( Ax = b \) where \( A \) is a `Tridiagonal` matrix.
      !! It also enables to solve a linear system with multiple right-hand sides.
      !!
      !! #### Syntax
      !!
      !! To solve a system with \( A \) being of type `Tridiagonal`:
      !!
      !! ```fortran
      !!    x = solve(A, b)
      !! ```
   ...
   interface inv
      pure module function inv_rdp(A) result(B)
         !! Utility function to compute the inverse of a `Tridiagonal` matrix.
         type(Tridiagonal), intent(in) :: A
         !! Input matrix.
   ...
   interface det
      ...
      !! determinant \(\det(A)\) where \(A\) is of type `Tridiagonal`.
      !!
      !! #### Syntax
      !!
      !! ```fortran
   ...
   interface trace
      ...
      !! of a matrix \( A \) of type `Tridiagonal`.
      !!
      !! #### Syntax
      !!
      !! ```fortran
   ...
   interface svdvals
      ...
      !! singular values of a `Tridiagonal` matrix \(A\).
      !!
      !! #### Syntax
      !!
      !! ```fortran
   ...
   interface svd
      ...
      !! the singular value decomposition of a `Tridiagonal` matrix \(A\).
      !!
      !! #### Syntax
      !!
      !! ```fortran
   ...
   interface eigvals
      ...
      !! eigenvalues of a real-valued matrix \( A \) whose type is `Tridiagonal`.
      !!
      !! #### Syntax
      !!
      !! ```fortran
   ...
   interface eig
      ...
      !! eigenvalues and eigenvectors of a real-valued matrix \(A\) whose type is `Tridiagonal`.
      !!
      !! #### Syntax
      !!
      !! ```fortran
   ...
   interface dense
      ...
      !! #### Arguments
      !!
      !! `A`   :  Matrix of `Tridiagonal`, `Bidiagonal`, `Tridiagonal` or `SymTridiagonal` type.
      !!          It is an `intent(in)` argument.
      !!
   ...
   interface size
      pure module function size_rdp(A, dim) result(arr_size)
         !! Utility function to return the size of `Tridiagonal` matrix along a given dimension.
         type(Tridiagonal), intent(in) :: A
         !! Input matrix.
   ...
   interface operator(*)
      pure module function scalar_multiplication_rdp(alpha, A) result(B)
         !! Utility function to perform a scalar multiplication with a `Tridiagonal` matrix.
